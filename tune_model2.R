...
full_train <- dplyr::slice_sample(full_train, prop = 0.02)  #comment this part out before moving to talapas.
...
# Model tuning
# There are three tunable parameters: neighbors, weight_func, and dist_power.
mod <- nearest_neighbor() %>%    # Model specification
  set_engine("kknn") %>%
  set_mode("classification") %>%
  set_args(neighbors = tune(),   # default neighbours = 5
           dist_power = tune()) # default dis_power = 2 i.e euclidean which is for continous predictors. Manhattan is 1 which is common for binary predictors. 

hyperparameters <- parameters(   # tuning the hyperparameters
  neighbors(range = c(1, 20)),
  dist_power())

grd <- grid_max_entropy(hyperparameters, size = 25)  # non-regular grid

tuning <- tune::tune_grid(mod, 
                          preprocessor = rec,
                          resamples = cv,
                          grid = grd)

saveRDS(tuning, "models/tuning.Rds")

#ggplot(grd, aes(neighbors, dist_power)) + # comment out when submitting to talapas
#  geom_point() +
#  theme_minimal()

#ggsave(here::here("plots", "non-regular-grid.pdf"),
#       width = 6.5,
#       height = 9)
